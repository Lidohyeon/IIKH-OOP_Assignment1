...
public:
     ...
     * 매개변수 생성자 (날짜 범위 지정)
     */
    PlanManager(const Date &start, const Date &end, char diffLevel = 'A')
    ...
    string getDifficultyDescription(Difficulty level) const
    {
        ...
        {
        case Difficulty::A:
            return "A (Advanced/Hard)";
        case Difficulty::B:
            return "B (Intermediate/Medium)";
        case Difficulty::C:
            return "C (Beginner/Easy)";
        default:
            return "Unknown";
        }
    }
    ...
    void generateBalancedPlan()
    {
        ...
        if (breakfastPool.empty() || lunchPool.empty() || dinnerPool.empty())
        {
            ...
            if (breakfastPool.empty())
                cout << "   - No easy recipes for breakfast." << endl;
            if (lunchPool.empty())
                cout << "   - No easy/medium recipes for lunch." << endl;
            if (dinnerPool.empty())
                cout << "   - No medium/hard recipes for dinner." << endl;
            return;
        }
